...
这次清晰度相关的入口在 `src/lib/mmdPetConfig.ts`：
...
- 思考时的 `...`、睡觉时的 `Zzz` 现在是 `.mmd-status-cue`，属于 MMD 渲染层旁边的状态提示，不再复用旧占位角色的 `.pet-effects`。

以后要做模型功能，优先改 `MmdPetModel.tsx` 和 `mmdPetConfig.ts`，不要回到旧 CSS 画身体那条路。
...
比较稳的下一步是新建一个 MMD action 层，例如 `src/lib/mmdActions.ts`，把状态映射成“根节点轻量动作 + 表情 morph + 可选 VMD/VPD”。这样以后扩展动作时不会污染 `Pet.tsx` 的点击/拖动逻辑，也不会回到 CSS 占位图时代。
